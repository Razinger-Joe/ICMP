    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">Executive Reporting</h1>
                    <p className="text-slate-500 dark:text-slate-400">Data aggregation and formal stakeholder exports</p>
                </div>
                <div className="flex items-center gap-2">
                    <Button className="bg-blue-600 hover:bg-blue-700 text-white gap-2 shadow-lg active:scale-95 transition-transform">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    ...
                    <Card key={i} className="border-none shadow-md bg-white dark:bg-slate-900 ring-1 ring-slate-200 dark:ring-slate-800">
            ...
            <Card className="border-none shadow-lg bg-white dark:bg-slate-900 ring-1 ring-slate-200 dark:ring-slate-800 overflow-hidden">
                <CardHeader className="flex flex-row items-center justify-between p-6">
                    <CardTitle className="text-sm font-semibold text-slate-500 uppercase tracking-widest leading-none">Recent Generated Reports</CardTitle>
                    <div className="flex gap-2">
                        <div className="relative hidden sm:block">
                            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" size={14} />
                            <input type="text" placeholder="Search reports..." className="bg-slate-50 dark:bg-slate-800 text-xs pl-8 pr-4 py-1.5 rounded-lg border-none focus:ring-1 focus:ring-blue-500 outline-none w-48" />
                        </div>
                        ...
                        <TableBody>
                            {reports.map((report) => (
                                <TableRow key={report.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <TableCell><Badge variant="secondary" className="text-[10px] font-bold uppercase">{report.type}</Badge></TableCell>
                                    <TableCell>
                                        <div className="flex items-center gap-1.5">
                                            <div className={cn(
                                                "h-1.5 w-1.5 rounded-full",
                                                report.status === 'Generated' ? "bg-emerald-500" : report.status === 'Pending' ? "bg-amber-500 animate-pulse" : "bg-slate-400"
                                            )} />
