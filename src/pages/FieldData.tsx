...
const devices = [
    { id: 'SEN-001', type: 'Climate', location: 'Section A - Basement', status: 'Online', signal: 'Strong', batteryPath: '92%' },
    { id: 'SEN-042', type: 'Vibration', location: 'Elevator Shaft 3', status: 'Online', signal: 'Good', batteryPath: '74%' },
    { id: 'PWR-102', type: 'Meter', location: 'Main Power Room', status: 'Online', signal: 'Strong', batteryPath: 'Main' },
    { id: 'WTR-205', type: 'Flow', location: 'Pump House B', status: 'Warning', signal: 'Weak', batteryPath: '45%' },
];
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-emerald-500">Field Telemetry</h1>
                    <p className="text-slate-500 dark:text-slate-400">Real-time IoT sensor network and site conditions</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    ...
                    <Card key={i} className="border-none shadow-md bg-white dark:bg-slate-900 overflow-hidden ring-1 ring-slate-200 dark:ring-slate-800">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className="lg:col-span-2 border-none shadow-lg bg-white dark:bg-slate-900 ring-1 ring-slate-200 dark:ring-slate-800">
                    <CardHeader>
                        <CardTitle className="text-sm font-semibold text-slate-500 uppercase tracking-widest">Site Telemetry Trends (Last 6 Hours)</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="h-[350px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={sensorData}>
                                    ...
                                    <Line type="monotone" dataKey="energy" stroke="#10b981" strokeWidth={4} dot={{ r: 0 }} />
                                    <Line type="monotone" dataKey="temp" stroke="#f59e0b" strokeWidth={2} dot={{ r: 0 }} />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </CardContent>
                                ...
                                <div key={device.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                    <div className="flex justify-between items-start mb-1">
                                        ...
                                        <Badge variant="outline" className={cn(
                                            "text-[10px] font-bold",
                                            device.status === 'Online' ? "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" : "text-rose-500 bg-rose-500/10 border-rose-500/20"
                                        )}>
                                            {device.status}
                                        </Badge>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between text-[10px]">
                                        ...
                                        <span className="text-slate-400">Power: <span className="text-white font-bold">{device.batteryPath}</span></span>
                                    </div>
                                </div>
                            ))}
                        </div>
